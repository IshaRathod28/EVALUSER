...
const Userdata = () => {
    ...
    const addToTable = async (e) => {
        ...
        // try {
        //   const res = await axios.post("http://localhost:2222/import", {
        //     code: code,
        //     firstname: firstname,
        //     lastname: lastname,
        //     email: email,
    ...
    const showuser = async () => {
        try {
            const res = await axios.get("http://localhost:2222/showuser")
            console.log(res.data);
            setUserdata(res.data);
    ...
    const handledelete = async (code) => {
        try {
            const res = await axios.delete("http://localhost:2222/userdelete", { params: { code } })
            console.log(res.data);
    ...
    const handelstatus = async (code) => {
        try {
            const res = await axios.get("http://localhost:2222/changestatus", { params: { code, dispstatus : dispstatus } })
        ...
        reader.onload = async ({ target }) => {
            const csv = Papa.parse(target.result, {
                ...
                complete: function (results) {
                    ...
                    const importeddata = axios.post("http://localhost:2222/import", results.data);
                        ...
                        {userdata.slice(curPage * 3, curPage * 3 + 3).map((udata, index) => (
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <img
                                            src={`http://localhost:2222/getimage/${findImageName(
                                                udata.photo
                                            )}`}
                                            style={{ height: "100px" }} />
                                    </td>
