...
console.log("User App is listening on Port 2222");
...
app.post('/adduser', async (req, res) => {
    ...
    try {
        const sqlQuery = `insert into ishauser_table (code,firstname,lastname,email,gender,hobbies,photo,country,dateadded,dateupdated,endeffdt,status) 
...
app.get("/showuser", async (req, res) => {
    try {
        const sqlQuery = `select * from ishauser_table where status = "A" `
        const ans = await queryRunner(sqlQuery);
        res.status(200).send(ans);
    } catch (err) {
        res.status(400).send(err);
...
app.get("/getuser", async (req, res) => {
    try {
        ...
        console.log(code);
        const sqlQuery = `select * from ishauser_table
        where code = "${code}"`
        const ans = await queryRunner(sqlQuery);
        res.status(200).send(ans);
...
app.post('/updatesetdata', async (req, res) => {
    ...
    try {
        const sqlQuery = `update ishauser_table set firstname = "${firstname}", lastname = "${lastname}",email = "${email}", gender = "${gender}", hobbies = "${hobbies}", photo = "${photo}",country = "${country}", dispstatus = "${dispstatus}", dateupdated = "${updateddate}" where code="${code}"`
        const ans = await queryRunner(sqlQuery);
        res.status(200).send(ans);
    } catch (err) {
        res.status(400).send(err);
...
app.delete("/userdelete", async (req, res) => {
    try {
        const code = req.query.code;
        const sqlquery = `update ishauser_table set status= "i" where code = "${code}"`
        const ans = await queryRunner(sqlquery);
        res.status(200).send("ans");
    }
    catch (err) {
...
app.get("/changestatus", async (req, res) => {
    try {
        ...
        if(status == "Active"){
            const sqlquery = `update ishauser_table set dispstatus= "Inactive" , dateupdated = "${date}"  where code = "${code}"`
            const ans = await queryRunner(sqlquery);
            res.status(200).send("ans");
          }
          if(status == "Inactive"){
            const sqlquery = `update ishauser_table set dispstatus= "Active" , dateupdated = "${date}"  where code = "${code}"`
            const ans = await queryRunner(sqlquery);
            res.status(200).send("ans");
          }  // const sqlquery2 = `update ishauser_table set dispstatus= "Active" where code = "${code}"`
...
app.get("/sortdata", async (req, res) => {
    try {
        ...
        const sqlquery = `select * from ishauser_table order by ${sort}`
...
app.post("/import", async (req, res) => {
    try {
        ...
        ans.map(async (item) => {
            let code = item.code;
            const ans = await queryRunner(`select * from ishauser_table where code="${code}"`)
            console.log(item, " ", ans)
            if (ans.length > 0) {
                const sqlQuery = `update ishauser_table set firstname = "${item.firstname}", lastname = "${item.lastname}",email = "${item.email}", gender = "${item.gender}", hobbies = "${item.hobbies}", photo = "${item.photo}",country = "${item.country}", dispstatus = "${item.dispstatus}", dateupdated = "${item.updateddate}" where code="${item.code}"`
                const ans = await queryRunner(sqlQuery);
                
            }
            else if (ans.length == 0) {
                let sqlQuery = `Insert into ishauser_table (code,firstname,lastname,email,gender,hobbies,photo,country,status,dateadded,dateupdated,endeffdt,dispstatus) values("${item.code}","${item.firstname}","${item.lastname}","${item.email}","${item.gender}","${item.hobbies}","${item.photo}","${item.country}","${item.status}","${dateadded}","${dateupdated}","${endeffdt}","${item.dispstatus}")`;
...
app.get("/filterdata", async (req, res) => {
    ...
    try {
        ...
        let sqlquery2 = `select * from ishauser_table  ` + sqlquery;
